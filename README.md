...
This analysis used over 9,000 Tweets on Apple and Google products to build a natural language processing (NLP) model to analyze consumer sentiment. 
...
* This graph below shows the relationship between the length of the postive and negative tweets. Looking at the graph it shows that the length of the tweet does not impact on whether a tweet will be labeled as positive, negative or no emotion based on the length.
...
* Include the the tweets from the "I can't tell" is can better prepare prepare for unseen data.
